...
def train_and_save_model():
    ...
    various machine learning models, including Logistic Regression, Naive Bayes, and Linear SVC.
    Hyperparameter tuning is conducted using GridSearchCV. The model demonstrating the best
    performance, based on accuracy metrics, is subsequently saved for future deployment.
    ...
    try:
        ...
        models = {
            ...
            "Linear SVC": OneVsRestClassifier(LinearSVC(random_state=42, dual='auto', max_iter=3000))
        }
        ...
        for name in models:
            ...
            pipeline = make_pipeline(
                TfidfVectorizer(stop_words='english', ngram_range=(1, 3), max_features=15000),
                models[name]
            )
...
class DetectorApp:
    ...
    def __init__(self, root, model):
        ...
        self.root.geometry("1050x700")  ### Adjusted window width to accommodate side-by-side layout
        self.root.configure(bg="#F0F0F0")  ### Configured a light background for the application window
        self.root.resizable(False, False)  ### Maintained fixed window dimensions to control layout consistency
        self._create_widgets()

    def _create_widgets(self):
        """Constructs and arranges all graphical user interface components."""
        ### --- Main Application Frame ---
        main_frame = tk.Frame(self.root, bg="#F0F0F0", padx=20, pady=20)  ### Utilizes a light background for the main frame
        main_frame.pack(expand=True, fill=tk.BOTH)

        ### --- Application Title ---
        ### Ensures the title is centrally aligned and rendered in black.
        title_label = tk.Label(main_frame, text="CommandGuard AI", font=("Helvetica", 24, "bold"),
                               bg="#F0F0F0", fg="#000000")  ### Set foreground color to black
        title_label.pack(pady=(0, 25), fill=tk.X)  ### `fill=tk.X` and default anchor ensure horizontal centering

        ### --- Horizontal Content Frame (Separates Input from Results/Analysis) ---
        content_frame = tk.Frame(main_frame, bg="#F0F0F0")  ### Employs a light background for the content frame
        content_frame.pack(expand=True, fill=tk.BOTH)

        ### --- Left Panel: Command Input Section ---
        input_panel_frame = tk.Frame(content_frame, bg="#F0F0F0", padx=10, pady=0)  ### Uses a light background for the input panel
        ...
        input_label_frame = tk.LabelFrame(input_panel_frame, text="Enter Command", font=("Helvetica", 12, "bold"),
                                          ...
                                          pady=10)  ### White background with dark grey text
        input_label_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 15))

        ### Provides instructional text positioned above the text input area
        ...
        prompt_input_instruction_label.pack(pady=(0, 5), fill=tk.X)  ### Incorporates a small vertical padding below

        self.text_area = scrolledtext.ScrolledText(input_label_frame, height=10, font=("Consolas", 11), bg="#F5F5F5",
                                                   ### Utilizes a very light grey for the text input area
                                                   fg="#222222", insertbackground="#000000", relief=tk.FLAT,
                                                   ### Configures dark text with a black cursor
                                                   borderwidth=1, padx=8, pady=8)
        self.text_area.pack(fill=tk.BOTH, expand=True)

        ### Scan button configured in black for visual prominence
        scan_button = tk.Button(input_panel_frame, text="Scan Command", font=("Helvetica", 13, "bold"),
                                command=self.check_prompt, bg="#000000", fg="#FFFFFF",  ### Black button with white text
                                activebackground="#333333", activeforeground="#FFFFFF",  ### Darker black when active
                                relief=tk.FLAT, pady=12, cursor="hand2")
        scan_button.pack(pady=(15, 0), fill=tk.X)

        ### --- Right Panel: Results and Analysis Display Sections ---
        info_panel_frame = tk.Frame(content_frame, bg="#F0F0F0", padx=10, pady=0)  ### Applies a light background to the information panel
        ...
        result_container_frame = tk.Frame(info_panel_frame, bg="#FFFFFF", bd=2, relief=tk.SOLID,
                                          ...
                                          highlightbackground="#CCCCCC", highlightthickness=1)  ### Features a light border
        result_container_frame.pack(fill=tk.X, pady=(0, 15), ipady=5)

        ### Text updated from "CLASSIFICATION RESULT" to "DETECTION OUTCOME" for clarity
        result_title_label = tk.Label(result_container_frame, text="DETECTION OUTCOME", font=("Helvetica", 12, "bold"),
                                      ...
                                      anchor="w")  ### Dark grey text on a white background
        result_title_label.pack(fill=tk.X, pady=(0, 5))

        ### Initial text is left empty, to be populated dynamically after command analysis
        self.predicted_label = tk.Label(result_container_frame, text="", font=("Helvetica", 16, "bold"),
                                        bg="#FFFFFF", fg="#222222", anchor="w", padx=10)  ### White background with dark text
        ...
        analysis_container_frame = tk.Frame(info_panel_frame, bg="#FFFFFF", bd=2, relief=tk.SOLID,
                                            ### White background with a solid border
                                            highlightbackground="#CCCCCC", highlightthickness=1)  ### Includes a light border
        ...
        ### New subheading for "Explanation"
        ...
        ### Explanation text area (ScrolledText for enhanced content handling)
        self.explanation_text_area = scrolledtext.ScrolledText(analysis_container_frame, height=5,
                                                               font=("Helvetica", 10),
                                                               bg="#F5F5F5", fg="#333333", relief=tk.FLAT,
                                                               borderwidth=0,
                                                               padx=10, pady=5, wrap=tk.WORD)  ### Configured for word wrapping
        self.explanation_text_area.config(state='disabled')  ### Disables direct editing of this text area

        ### New subheading for "Recommendation"
        self.recommendation_title = tk.Label(analysis_container_frame, text="Recommendation:",
                                             font=("Helvetica", 10, "bold"),
                                             bg="#FFFFFF", fg="#444444", anchor="nw", padx=10)
        ### Recommendation text area (ScrolledText for enhanced content handling)
        self.recommendation_text_area = scrolledtext.ScrolledText(analysis_container_frame, height=5,
                                                                  ...
                                                                  borderwidth=0,
                                                                  padx=10, pady=5,
                                                                  wrap=tk.WORD)  ### Default green color, subject to change based on classification
        self.recommendation_text_area.config(state='disabled')  ### Disables direct editing of this text area

        ### Packs explanation and recommendation text areas with `expand=True` to optimally distribute available space
        ### within the `analysis_container_frame`
        self.explanation_text_area.pack(fill=tk.BOTH, expand=True)
        self.recommendation_text_area.pack(fill=tk.BOTH, expand=True)

        ### Initially conceals detailed analysis components until their display is required
    ...
    def check_prompt(self):
        ...
        if not prompt_text:
            self.predicted_label.config(text="Please input a command for analysis.", fg="#FFA500")  ### Orange for warning indication
            self.confidence_label.config(text="N/A", fg="#555555")  ### Dark grey for non-applicable status
            self.hide_analysis_section()
            return
        ...
        try:
            ...
            ### Color definitions for text within the analysis summary display
            default_exp_color = "#333333"  ### Default dark grey for explanation text

            if predicted_class == 2:  ### Indicates a malicious classification
                result_text = "MALICIOUS"
                result_color = "#CC0000"  ### Darker red for malicious classification result
                rec_color_for_analysis = "#CC0000"  ### Red for recommendations pertinent to malicious classification
                self.show_analysis(prompt_text, "malicious", default_exp_color, rec_color_for_analysis)
            elif predicted_class == 1:  ### Indicates a suspicious classification
                result_text = "SUSPICIOUS"
                result_color = "#FFA500"  ### Vibrant orange for suspicious classification result
                rec_color_for_analysis = "#FFA500"  ### Orange for recommendations pertinent to suspicious classification
                self.show_analysis(prompt_text, "suspicious", default_exp_color, rec_color_for_analysis)
            else:  ### Indicates a benign classification
                result_text = "BENIGN"
                result_color = "#008000"  ### Darker green for benign classification result
                rec_color_for_analysis = "#008000"  ### Green for recommendations pertinent to benign classification
                self.show_benign_recommendation(default_exp_color, rec_color_for_analysis)

            ### Updates labels as per user request specifications
            ...
            self.predicted_label.config(text=f"Error", fg="#CC0000")  ### Red for error indication
            self.confidence_label.config(text=f"N/A", fg="#555555")
            ### Displays error information within the analysis summary
    ...
    def show_analysis(self, prompt_text, level, exp_color, rec_color):
        ...
        for keyword, (exp, rec) in ANALYSIS_MAP.items():
            if keyword in prompt_lower:
                if (exp, rec) not in found_keywords:  ### Prevents duplicate entries if multiple keywords map to the same explanation
    ...
    def show_analysis_section(self):
        ...
        ### Uses `pady` in `pack` for consistent vertical spacing
        self.explanation_title.pack(fill=tk.X, pady=(5, 0))  ### Applies vertical padding here
        self.explanation_text_area.pack(fill=tk.BOTH, expand=True)
        self.recommendation_title.pack(fill=tk.X, pady=(10, 0))  ### Applies vertical padding here
        self.recommendation_text_area.pack(fill=tk.BOTH, expand=True)
...
if __name__ == "__main__":
    ### To ensure a fresh model training cycle, any existing model file is removed if detected.
    ### This action also facilitates a clear demonstration of the training process for evaluation purposes.
